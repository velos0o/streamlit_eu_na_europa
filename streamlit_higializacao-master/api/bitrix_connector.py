...
try:
    load_dotenv()
    st.sidebar.success("Variáveis de ambiente carregadas com sucesso.")
except Exception as e:
    st.sidebar.warning(f"Erro ao carregar variáveis de ambiente: {str(e)}")
...
def get_credentials():
    ...
    try:
        ...
        if hasattr(st, 'secrets'):
            if 'BITRIX_TOKEN' in st.secrets:
                ...
                source = "secrets"
                st.sidebar.success("Credenciais obtidas de Streamlit Secrets.")
    except Exception as e:
        st.sidebar.warning(f"Erro ao acessar Streamlit Secrets: {str(e)}")
    
    # Se não conseguir dos secrets, tentar variáveis de ambiente
    if not token or not url:
        try:
            ...
            if token and url:
                source = "env"
                st.sidebar.success("Credenciais obtidas das variáveis de ambiente.")
        except Exception as e:
            st.sidebar.warning(f"Erro ao acessar variáveis de ambiente: {str(e)}")
    ...
    if not token or not url:
        ...
        source = "padrão"
        st.sidebar.warning("Usando credenciais padrão. Isso pode não funcionar em produção.")
    
    # Registrar na sessão a fonte das credenciais
    st.session_state['credential_source'] = source
